...
def export_videos(nuscenes: NuScenes, out_dir: str):
    ...
    for scene_token in scene_tokens:
        ...
        if not os.path.exists(out_path):
            nuscenes.render_scene_channel(scene['token'],
                                          'CAM_FRONT',
                                          out_path=out_path)
...
def post_process_coords(corner_coords: List,
    ...
    """
    Get the intersection of the convex hull of the reprojected bbox corners and
     the image canvas, return None if no intersection.
    :param corner_coords: Corner coordinates of reprojected bounding box.
    :param imsize: Size of the image canvas.
    :return: Intersection of the convex hull of the 2D box corners and the
    image canvas.
    """
...
def generate_record(ann_rec: dict,
    ...
    """
    Generate one 2D annotation record given various informations on top of the
...
def get_2d_boxes(sample_data_token: str,
    ...
    :return: List of 2D annotation record that belongs to the
    input `sample_data_token`
    """
    ...
    if not sd_rec['is_key_frame']:
        raise ValueError('The 2D re-projections are available only '
                         'for keyframes.')

    s_rec = nusc.get('sample', sd_rec['sample_token'])

    # Get the calibrated sensor and ego pose record to get the
    ...
    ann_recs = [nusc.get('sample_annotation', token) for
                token in s_rec['anns']]
    ...
    for ann_rec in ann_recs:
        ...
        annotation_box = nusc.get_box(ann_rec['token'])

        # Move them to the ego-pose frame.
        annotation_box.translate(-np.array(pose_rec['translation']))
        annotation_box.rotate(Quaternion(pose_rec['rotation']).inverse)

        # Move them to the calibrated sensor frame.
        annotation_box.translate(-np.array(cs_rec['translation']))
        annotation_box.rotate(Quaternion(cs_rec['rotation']).inverse)

        # Filter out the corners that are not in front of the
        # calibrated sensor.
        corners_3d = annotation_box.corners()
        in_front = np.argwhere(corners_3d[2, :] > 0).flatten()
        corners_3d = corners_3d[:, in_front]

        # Project 3d box to 2d.
        corner_coords = view_points(corners_3d,
                                    camera_intrinsic, True).T[:, :2].tolist()

        # Keep only corners that fall within the image.
        final_coords = post_process_coords(corner_coords)

        # Skip if the convex hull of the re-projected corners does not
...
def main(args):
    """Generates 2D re-projections of the 3D bounding boxes present
    in the dataset."""
    ...
    with open(os.path.join(args.dataroot,
                           args.version, args.filename), 'w') as fh:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Export 2D annotations from reprojections to a '
                    '.json file.',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--dataroot', type=str, default='mini',
                        help="Path where nuScenes is saved.")
    parser.add_argument('--version', type=str, default='v1.0-mini',
                        help='Dataset version.')
    parser.add_argument('--filename', type=str, default='output.json',
                        help='Output filename.')
    parser.add_argument('--visibilities', type=str,
                        default=['', '1', '2', '3', '4'],
                        help='Visibility bins, the higher the number '
                             'the higher the visibility.', nargs='+')
    parser.add_argument('--image_limit', type=int, default=-1,
                        help='Number of images to process or -1 '
                             'to process all.')
    arguments = parser.parse_args()

    nusc = NuScenes(dataroot=arguments.dataroot, version=arguments.version)
    main(arguments)
