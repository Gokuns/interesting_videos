...
"")
    ...
    def openVideo(self, text):
        ...
        if fileName != config.argument_defaults['export_path']+'/'+'Select video...':
            ...
            selected_video = dataset.find_video_from_path(text)
            self.progress1.setValue(selected_video.number_of_people)
            self.progress2.setValue(selected_video.number_of_vehicles)
